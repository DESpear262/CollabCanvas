...
export function ConnectionStatus() {
  ...
  return (
    <div className="cc-conn" style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 2 }}>
      <div className="cc-conn-row" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span className="cc-conn-dot" aria-hidden style={{ width: 8, height: 8, borderRadius: 999, background: color, boxShadow: `0 0 0 2px ${bg}` }} />
        <span className="cc-conn-label" style={{ fontSize: 12, color: '#4b5563' }}>{label}</span>
      </div>
      {showToast ? (
        <div className="cc-conn-toast" style={{ fontSize: 12, color: '#065f46', background: 'rgba(16,185,129,0.12)', border: '1px solid #a7f3d0', padding: '4px 8px', borderRadius: 6, width: 'fit-content' }}>
          Reconnected
        </div>
      ) : null}
    </div>
